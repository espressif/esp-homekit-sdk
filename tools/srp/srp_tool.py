...
def gen_x( hash_class, salt, username, password):
    ...
    if _no_username_in_x:
        username = six.b('')
    return int(H( hash_class, salt, binascii.a2b_hex(H( hash_class, username + six.b(':') + password))),16)
    #return H( hash_class, salt, H( hash_class, username + six.b(':') + password ))




def create_salted_verification_key( username, password, hash_alg=SHA1, ng_type=NG_2048, n_hex=None, g_hex=None, salt_len=4, salt=None ):
